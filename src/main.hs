...
doStuff opt (Pop a b) = genPopulationMap2 opt a b
...
doStuff opt (Heatmap name i t st) = renderDiagram name $ populationMapToDiagram (fromIntegral $ mapRange opt) $ VB.map V.length $ peopleMap (range*range) $ V.filter comp g'
	where
	comp = case t of
		"cult" -> (== stringToCulture st).culture
		"prof" -> (== stringToProfession st).profession
...
data OptionsType = Pop Int Int | Cult Int Int | Prof Int Int | Perf String String [Int] | Connections Int String String | Heatmap String Int String String | OptionsFile String | Help | Flag String deriving (Eq)
...
genPopulationMap2 :: Options -> Int -> Int -> IO ()
genPopulationMap2 opt n1 n2 = mapM_ (\index -> do
...
genPopulationMap opt n1 n2 = let range = [read n1..read n2] in mapM_ (\(index, name) -> do
	let (g,_,_) = createGeneration opt index
	let g' = V.filter alive g
	let positions = let list = V.filter (/=(0,0)) $ V.map position g' in [[((x,y), V.length $ V.filter (\(x',y') -> x == x' && y == y') list) | x <- [0..mapRange opt]] | y <- [0..mapRange opt]]
	let f x
		| x == 0 = 0
		| x < 10 = 50
		| x < 50 = 50 + x * 3
...
genCultureMap opt n1 n2 = let range = [n1..n2] in mapM_ (\(index, name) -> do
	...
	let (g,_,_) = createGeneration opt index
	let !g' = V.filter alive g
	let !positions = [[((x,y), [f p c | c <- allCultures])  | x <- [0..mapRange opt], let p = V.filter ((\(x',y') -> x == x' && y == y').position) g'] | y <- [0..mapRange opt]]
...
genProfessionMap opt n1 n2 = let range = [n1..n2] in mapM_ (\(index, name) -> do
	let f :: People -> Int; f l
		| V.null l = 0
		| otherwise = V.foldr (\x list -> list + (*) 100 (round $ (professionValue opt) $ profession x)) 0 l `div` V.length l
	let (g,_,_) = createGeneration opt index
	let !g' = V.filter alive g
	let !positions = [[((x,y), f p) | x <- [0..mapRange opt], let p = V.filter ((\(x',y') -> x == x' && y == y').position) g'] | y <- [0..mapRange opt]]
